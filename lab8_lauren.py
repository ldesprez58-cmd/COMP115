...
assert remove_duplicates_set("aaaa") == "a"
...
def gem_counting(stones, gems):
    ...
    Parameters:
        stones (str): Characters representing collected stones.
        gems (str): Characters representing gem types.

    Returns:
        int: The number of stones that are gems.
    """
    gem_set = set(gems)
    count = 0
...
def students_id(ids):
    """
    Counts the number of students who use the shuttle service.

    Parameters:
    ids(list): A list of student ID strings.

    Returns:
    int: The number of unique student IDs in the list.
    """
    # count=0
    # seen_set = set()
    # for id in ids:
...
def students_id_occurrences(ids):
    """
    Counts how many times each student ID appears in the list.

    Parameters:
    ids (list): A list of student ID strings.

    Returns:
    dict: A dictionary where the keys are students IDs and the values
    are the number occurences of each ID.
...
def word_frequency(paragraph):
    ...
    Parameters:
        paragraph (str): A string containing a sentence or paragraph.

    Returns:
        dict: A dictionary where the keys are words from the paragraph
